...
function App() {
  ...
  const filterMovieByGenre=(x)=>{
    let mov=[...filteredMovie];

    console.log(`filtered item by ${x}`)

    let newArr=mov.filter((item)=>{
      
      return item.genre===x
    });

    setFilteredMovie(newArr);
  }
        ...
        <div className='genreBox'>
          ...
          <div className='genreButtonContainer'>
          {
            genres.map((item,index)=>(
              <button className='genreButton' key={index} onClick={()=>filterMovieByGenre(item)}>
                {item}
              </button>
            ))
          }
